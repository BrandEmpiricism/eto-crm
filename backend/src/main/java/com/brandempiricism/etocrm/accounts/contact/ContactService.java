...
public class ContactService {
 ...
 @Transactional ContactView update(UUID accountId,UUID id,AccountApplicationApi.CreateContact command,String actor){var contact=find(accountId,id);contact.name=required(command.name(),"Contact name is required.");contact.email=email(command.email());contact.role=clean(command.role());contact.notes=clean(command.notes());contact.updatedAt=Instant.now();contact.updatedBy=actor;return view(contact);}
